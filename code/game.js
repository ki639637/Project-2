...
function Bees(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.7, -0.1));
  this.size = new Vector(0.3, 0.3);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 1;
}
Bees.prototype.type = "bees";
...
Bees.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
   ...
   if ( type == "levelWin" && this.status == null) {
		this.status = "won"
		this.finishDelay = 1;
  }
   if ( type == "levelWin" && this.status == null) {
	 ...
	 this.player.pos = new Vector (6,15);
	}
